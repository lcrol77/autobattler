...
func _reset_unit_to_starting_position(starting_pos: Vector2, unit: Unit) -> void:
	var i := _get_play_area_for_position(starting_pos)
	var tile := play_areas[i].get_tile_from_global(starting_pos)
	unit.reset_after_dragging(starting_pos)
	play_areas[i].unit_grid.add_unit(tile, unit) # resposible for resetting unit back to its unit original unit grid slot
...
func _on_unit_dropped(starting_pos: Vector2, unit: Unit) -> void:
	_set_highlighter(false)
	var old_area_index := _get_play_area_for_position(starting_pos)
	...
	var old_tile := old_area.get_tile_from_global(starting_pos)
